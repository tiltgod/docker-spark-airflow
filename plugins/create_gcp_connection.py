...
def create_gcp_connection(gcp_connection_config):
    ...
    print("check: ", ",".join(gcp_connection_config.scope))
    extra_field = {
        "extra__google_cloud_platform__scope": ",".join(gcp_connection_config.scope),
        "extra__google_cloud_platform__project": gcp_connection_config.project,
        "extra__google_cloud_platform__key_path": gcp_connection_config.key_path
    }
    ...
    if session.query(Connection).filter(Connection.conn_id == new_conn.conn_id).first():
        print("check: ", session.query(Connection).filter(Connection.conn_id == new_conn.conn_id).first())
